...
def plot_production_date_gmm(df, n_components=3):
    ...
    df['PRODUCTION_DIFF'] = (df['PROD_ORDER_COMPL_DATE'] - df['PROD_ORDER_END_DATE']).dt.days
    ...
    plt.xlabel('Produktionsdifferenz (Fertigstellung - Plan) in Tagen')
    plt.ylabel('Dichte')
    plt.title('Histogramm der Produktionsdifferenz mit Gaussian Mixture Model (GMM) Fitting')
